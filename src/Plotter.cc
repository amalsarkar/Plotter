...
void Plotter::setYAxisTop(TH1* datahist, TH1* error, double ratio, THStack* hs) {
  ...
  if(styler.getDivideBins()) {
   //int bin_width = datahist->GetBinWidth(1);
   //std::string s = std::to_string(bin_width);
   //std::string y_title_name = "Events/"+s+" GeV";
  //yaxis->SetTitle(y_title_name.c_str());////get axis title stuff
    yaxis->SetTitle("Events/GeV");////get axis title stuff
  }
